...
public sealed class WeaveEnvelopeTests
{
    ...
    {
        var called = false;
        await new WeaveMorph<int, int>(new AsWeave<int, int>(x => x))
    ...
    {
        var received = 0;
        await new WeaveMorph<int, int>(new AsWeave<int, int>(x => x * 2))
    ...
    public async Task Weave_DelegatesToOrigin()
        => Assert.Equal("42", await new WeaveMorph<int, int>(new AsWeave<int, int>(x => x * 2))
